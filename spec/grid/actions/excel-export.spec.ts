...
describe('Theme', () => {
    ...
    it('fabric', (done) => {
        gridObj.excelExport({ theme: 'fabric' });
        setTimeout(() => {
            expect('').toBe('');
            done();
        }, 500);
    ...
    it('bootstrap', (done) => {
        gridObj.excelExport({ theme: 'bootstrap' });
        setTimeout(() => {
            expect('').toBe('');
            done();
        }, 500);
...
//                     { field: 'ShipRegion', width: 140 },
//                     { field: 'Verified', width: 140 },
//                     { field: 'ShipPostalCode', width: 140 },
//                 ],
//                 aggregates: [{
//                     columns: [
//                         {
...
//     it('Grouped Grid Exporting', (done) => {
...
//     });
// });
describe('Grouping export testing', () => {
...
describe('Single Level Grouping export testing', () => {
...
describe('excel export testing', () => {
    ...
    it('Stacked header with single level', (done) => {
        document.body.appendChild(elem);
        gridObj = new Grid(
            {
        ...
        gridObj.excelExport(undefined);

        //grid.excelExport(); // <- default grid exporting.
        //  }
        // }
        setTimeout(() => {
            expect('').toBe('');
    ...
    it('Stacked header with multiple level', (done) => {
        document.body.appendChild(elem);
        gridObj = new Grid(
            {
                dataSource: data.slice(0, 30),
    ...
    it('stacked header without header text in column', (done) => {
        document.body.appendChild(elem);
        gridObj = new Grid(
            {
                dataSource: data.slice(0, 30),
    ...
    it('stacked header with single level without header text', (done) => {
        document.body.appendChild(elem);
        gridObj = new Grid(
            {
                dataSource: data.slice(0, 30),
    ...
    it('footersRows&headerRows=undefined', (done) => {
        document.body.appendChild(elem);
        gridObj = new Grid(
            {
                dataSource: data.slice(0, 30),
        ...
        gridObj.dataBind();
        let excelExportProperties: any = {
            multipleExport: { type: '', blankRows: 3 },
            header: {
                headerRows: undefined,
                rows: [
                    { cells: [{ colSpan: 6, value: "Northwind Traders", style: { fontColor: '#C67878', fontSize: 20, hAlign: 'center', bold: true, } }] },
    ...
    it('Grid format skeleton without type', (done) => {
        document.body.appendChild(elem);
        gridObj = new Grid(
            {
                dataSource: data.slice(0, 2),
