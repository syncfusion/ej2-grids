...
class ContentRender {
    ...
    refreshContentRows(args = {}) {
        ...
        getUpdateUsingRaf(() => {
            this.parent.trigger(beforeFragAppend, {});
            remove(this.tbody);
...
class Selection {
    ...
    addEventListener_checkbox() {
        this.parent.on(beforeFragAppend, this.beforeFragAppend, this);
    ...
    removeEventListener_checkbox() {
        this.parent.off(beforeFragAppend, this.beforeFragAppend);
...
class Resize {
    ...
    wireEvents() {
        ...
        EventHandler.add(this.parent.getHeaderTable(), dblclick, this.callAutoFit, this);
    }
    unwireEvents() {
        EventHandler.remove(this.parent.getHeaderTable(), Browser.touchStartEvent, this.resizeStart);
        EventHandler.remove(this.parent.getHeaderTable(), dblclick, this.callAutoFit);
    }
    getResizeHandlers() {
...
class EditRender {
    ...
    getEditElements(args) {
        ...
        for (let i = 0, len = cols.length; i < len; i++) {
            ...
            let isComplexField = !isNullOrUndefined(col.field) && col.field.split('.').length > 1;
            let splits = !isNullOrUndefined(col.field) && col.field.split('.');
...
class BooleanEditCell {
    ...
    create(args) {
        ...
        let isComplexField = !isNullOrUndefined(args.column.field) && args.column.field.split('.').length > 1;
        let splits = !isNullOrUndefined(args.column.field) && args.column.field.split('.');
...
class BatchEdit {
    ...
    setChanges(rowObj, field, value) {
        let isComplexField = !isNullOrUndefined(field) && field.split('.').length > 1;
        let splits = !isNullOrUndefined(field) && field.split('.');
        if (!rowObj.changes) {
    ...
    saveCell(isForceSave) {
        ...
        let isComplexField = !isNullOrUndefined(column.field) && column.field.split('.').length > 1;
        let splits = !isNullOrUndefined(column.field) && column.field.split('.');
    ...
    setWrapHeight(fRows, mRows, isModeChg, isContReset, isStackedHdr) {
        ...
        if (isWrap) {
            ...
            if (movableContentHeight > frozenContentHeight) {
                this.parent.element.querySelector('.e-frozencontent').style.height = movableContentHeight + 'px';
            }
            if (frozenContentHeight > movableContentHeight) {
                this.parent.element.querySelector('.e-movablecontent').style.height = frozenContentHeight + 'px';
            }
        }
    }
