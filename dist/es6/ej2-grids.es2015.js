...
class EditRender {
    ...
    getEditElements(args) {
        ...
        for (let i = 0, len = cols.length; i < len; i++) {
            ...
            let isComplexField = col.field.split('.').length > 1;
...
class BooleanEditCell {
    ...
    create(args) {
        ...
        let isComplexField = args.column.field.split('.').length > 1;
        let splits = args.column.field.split('.');
...
class BatchEdit {
    ...
    setChanges(rowObj, field, value) {
        let isComplexField = field.split('.').length > 1;
    ...
    saveCell(isForceSave) {
        ...
        let isComplexField = column.field.split('.').length > 1;
