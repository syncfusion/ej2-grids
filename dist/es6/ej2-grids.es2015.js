...
class Data {
    ...
    sortQuery(query) {
        ...
        if ((gObj.allowSorting || gObj.allowGrouping) && gObj.sortSettings.columns.length) {
            ...
            for (let i = columns.length - 1; i > -1; i--) {
                let col = gObj.getColumnByField(columns[i].field);
                if (gObj.groupSettings.columns.indexOf(columns[i].field) === -1) {
                    col.setSortDirection(columns[i].direction);
                    let fn = col.sortComparer ? col.sortComparer.bind(col) : columns[i].direction;
                    query.sortBy(col.field, fn);
                }
                else {
                    sortGrp.push({ direction: columns[i].direction, field: col.field });
                }
            }
    ...
    searchQuery(query) {
        ...
        let fields = sSettings.fields.length ? sSettings.fields : this.parent.getColumnFieldNames();
        let predicateList = [];
        if (this.parent.searchSettings.key.length) {
            if (this.parent.getForeignKeyColumns().length) {
                fields.forEach((columnName) => {
                    let column = this.parent.getColumnByField(columnName);
    ...
    filterQuery(query, column, skipFoerign) {
        ...
        if (gObj.allowFiltering && gObj.filterSettings.columns.length) {
            ...
            if (gObj.filterSettings.type === 'checkbox' || gObj.filterSettings.type === 'excel') {
                let excelPredicate = CheckBoxFilter.getPredicate(columns);
                ...
                for (let col of columns) {
                    let column = gObj.getColumnByField(col.field) ||
...
class ContentFocus {
    ...
    jump(action, current) {
        let frozenSwap = this.parent.getFrozenColumns() > 0 &&
    ...
    getCurrent(previous = [], swap, active, action, isHeader) {
        let current = [];
    ...
    generateRows(rows, optionals) {
        ...
        let defaultLen = this.parent.allowFiltering && this.parent.filterSettings.type === 'filterbar' ? 2 : 1;
    ...
    isFocusChange(e, index, table, isHeader) {
        if (index[0] < 0 || index[1] < 0) {
            return true;
        }
        let cell = getValue(`${index[0]}.cells.${index[1]}`, table.rows);
        if (!cell) {
            return true;
        }
...
class Selection {
    ...
    clearRowSelection() {
        if (this.isRowSelected) {
            ...
            for (let i = 0, len = this.selectedRowIndexes.length; i < len; i++) {
                let row = gObj.getRowByIndex(this.selectedRowIndexes[i]);
    ...
    addCellsToSelection(cellIndexes) {
        ...
        let gObj = this.parent;
        let frzCols = gObj.getFrozenColumns();
        let selectedCell = this.getSelectedMovableCell(cellIndexes[0]);
        if (!selectedCell) {
            selectedCell = gObj.getCellFromIndex(cellIndexes[0].rowIndex, this.getColIndex(cellIndexes[0].rowIndex, cellIndexes[0].cellIndex));
        }
        let index;
    ...
    cellDeselect(type, cellIndexes, data, cells, foreignKeyData$$1) {
        ...
        this.parent.trigger(type, {
            cells: cells, data: data, cellIndexes: cellIndexes, foreignKeyData: foreignKeyData$$1
        });
    }
    ...
    onCellFocused(e) {
        ...
        let headerAction = (e.container.isHeader && e.element.tagName !== 'TD' && !e.element.closest('.e-rowcell'))
    ...
    keyPressed(e) {
        ...
        if (!this.parent.isEdit && (e.action === 'enter' || e.action === 'ctrlEnter' || e.action === 'shiftEnter')
            ...
            if (!target || (target && !target.classList.contains('e-headercell')) ||
                (this.parent.getFrozenColumns() || this.parent.frozenRows || isNullOrUndefined(target))) {
                return;
            }
...
class Filter {
    ...
    columnPositionChanged(e) {
        ...
        let filterCells = [].slice.call(this.parent.getHeaderContent().querySelectorAll('.e-filterbarcell'));
...
class Resize {
    ...
    resizing(e) {
        if (this.parent.allowTextWrap && this.parent.getFrozenColumns()) {
            this.parent.notify(freezeRender, { case: 'textwrap' });
...
class Edit {
    ...
    createTooltip(element, error, name, display) {
        ...
        let table = this.parent.editSettings.mode !== 'dialog' ?
        ...
        let left = this.parent.editSettings.mode !== 'dialog' ?
    ...
    processRecordRows(gObj, record, headerRow, level) {
        ...
        for (let r = 0; r < rLen; r++) {
            ...
            for (let c = 0, len = headerRow.columns.length; c < len; c++) {
                /* tslint:disable-next-line:no-any */
                let value = getValue(headerRow.columns[c].field, record[r]);
    ...
    processRecord(border, columns, gObj, dataSource, pdfGrid, groupIndex) {
        ...
        for (let items of dataSource) {
            ...
            for (let j = 0; j < columns.length; j++) {
                /* tslint:disable:no-any */
                let value = getValue(columns[j].field, items) || '';
