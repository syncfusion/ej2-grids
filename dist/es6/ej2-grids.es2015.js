...
/** @hidden */
function getComplexValue(rowData, field) {
    let length = field.split('.').length;
    let splits = field.split('.');
    let complexData = rowData[splits[0]];
    let complexValue = complexData[splits[1]];
...
class CheckBoxFilter {
    ...
    refreshCheckboxes() {
        ...
        let operator = this.options.type ? 'startswith' : 'contains';
        let matchCase = true;
...
class Data {
    ...
    getStateEventArgument(query) {
        ...
        let final = Object.assign(data, state.pvtData);
        return final;
    }
...
class SummaryModelGenerator {
    ...
    setTemplate(column, data, single) {
        ...
        types.forEach((type) => {
            ...
            let val = type !== 'Custom' && group.aggregates && key in group.aggregates ? group.aggregates[key] :
...
class ContentRender {
    ...
    refreshContentRows(args = {}) {
        ...
        getUpdateUsingRaf(() => {
            this.parent.notify(beforeFragAppend, {});
            remove(this.tbody);
...
class CellRenderer {
    ...
    refreshCell(cell, data, attributes$$1) {
        ...
        if (column.type === 'boolean' && column.displayAsCheckBox) {
            let checked = isNaN(parseInt(value.toString(), 10)) ? value === 'true' : parseInt(value.toString(), 10) > 0;
            let checkWrap = createCheckBox(false, { checked: checked, label: ' ' });
            node.innerHTML = '';
...
class HeaderCellRenderer extends CellRenderer {
    ...
    prepareHeader(cell, node, fltrMenuEle) {
        ...
        let colIndex = gridObj.getColumnIndexByField(column.field);
...
class Selection {
    ...
    updatePersistCollection(selectedRow, chkState) {
        if (this.parent.isPersistSelection && !isNullOrUndefined(selectedRow)) {
            ...
            if (chkState) {
                ...
                let index;
                let isPresent = this.persistSelectedData.some((data, i) => {
                    index = i;
    ...
    rowDeselect(type, rowIndex, data, row, foreignKeyData$$1, mRow) {
        ...
        let rowDeselectObj = { rowIndex: rowIndex, data: data, row: row, foreignKeyData: foreignKeyData$$1, cancel: false };
...
let Grid = class Grid extends Component {
    ...
    destroy() {
        ...
        let modules = ['renderModule', 'headerModule', 'contentModule', 'valueFormatterService',
...
class Sort {
    ...
    refreshSortSettings() {
        this.sortedColumns = [];
        this.sortSettings.columns.forEach((e) => {
...
class BooleanFilterUI {
    ...
    create(args) {
        let isForeignColumn = args.column.isForeignColumn();
        let dataSource = isForeignColumn ? args.column.dataSource : this.parent.dataSource;
        let fields = isForeignColumn ? args.column.foreignKeyValue : args.column.field;
...
class ExcelFilter extends CheckBoxFilter {
    ...
    renderMatchCase(column, tr, matchCase, elementId, predicates) {
        ...
        let checkbox = new CheckBox({ label: this.getLocalizedLabel('MatchCase'),
...
class Filter {
    ...
    refreshFilterSettings() {
        if (this.filterSettings.type === 'FilterBar') {
            ...
            if (this.filterSettings.columns.length === 0) {
                let col = this.parent.getColumns();
                for (let i = 0; i < col.length; i++) {
                    let filterElement = this.getFilterBarElement(col[i].field);
    ...
    getFilterBarElement(col) {
        let selector = '[id=\'' + col + '_filterBarcell\']';
        let filterElement;
...
class EditRender {
    ...
    getEditElements(args) {
        ...
        for (let i = 0, len = cols.length; i < len; i++) {
            ...
            let isComplex = !isNullOrUndefined(col.field) && isComplexField(col.field);
            let complexFieldName = !isNullOrUndefined(col.field) && getComplexFieldID(col.field);
...
class BooleanEditCell {
    ...
    create(args) {
        ...
        let isComplex = !isNullOrUndefined(args.column.field) && isComplexField(args.column.field);
        let complexFieldName = !isNullOrUndefined(args.column.field) && getComplexFieldID(args.column.field);
    ...
    write(args) {
        ...
        let chkState;
        let isComplex = !isNullOrUndefined(args.column.field) && isComplexField(args.column.field);
        let isAddRow = args.requestType === 'add' || args.row.classList.contains('e-addedrow');
...
class NormalEdit {
    ...
    refreshRow(data) {
        ...
        if (rowObj) {
            ...
            if (frzCols) {
                let uid;
                let tr = this.parent.element.querySelector('[data-uid=' + rowObj.uid + ']');
...
class BatchEdit {
    ...
    editCell(index, field, isAdd) {
        ...
        if (gObj.editSettings.allowEditing && col.allowEditing) {
            ...
            if (frzCols && colIdx >= frzCols) {
                ...
                let mRowData = this.parent.getRowObjectFromUID(this.parent.getMovableRows()[index].getAttribute('data-uid'));
    ...
    setChanges(rowObj, field, value) {
        let isComplex = !isNullOrUndefined(field) && isComplexField(field);
        if (!rowObj.changes) {
    ...
    saveCell(isForceSave) {
        ...
        let isComplex = !isNullOrUndefined(column.field) && isComplexField(column.field);
        let cloneEditedData = extend({}, editedData);
        editedData = extend({}, editedData, this.cellDetails.rowData);
        if (isComplex) {
            let splits = column.field.split('.');
            let cloneComplexData = extend({}, this.cellDetails.rowData[splits[0]]);
...
class DatePickerEditCell {
    ...
    create(args) {
        /* tslint:disable-next-line:no-any */
        let complexFieldName = getComplexFieldID(args.column.field);
        let isComplex = isComplexField(args.column.field);
        return createElement('input', {
            className: 'e-field', attrs: {
                id: isComplex ? this.parent.element.id + complexFieldName : this.parent.element.id + args.column.field,
                name: isComplex ? complexFieldName : args.column.field, type: 'text', 'e-mappinguid': args.column.uid
...
class ExcelExport {
    ...
    processRecordContent(gObj, returnType, headerRow, isMultipleExport, currentViewRecords) {
        ...
        if (!isMultipleExport) {
            if (this.isCsvExport) {
                let book = new Workbook(this.book, 'csv', gObj.locale);
                if (!this.isBlob) {
                ...
                let book = new Workbook(this.book, 'xlsx', gObj.locale);
                if (!this.isBlob) {
...
class FreezeRender extends HeaderRender {
    ...
    setWrapHeight(fRows, mRows, isModeChg, isContReset, isStackedHdr) {
        ...
        if (isWrap) {
            let movableContentHeight = this.parent.element.querySelector('.e-movablecontent').getBoundingClientRect().height;
            let frozenContentHeight = this.parent.element.querySelector('.e-frozencontent').getBoundingClientRect().height;
...
class ForeignKey extends Data {
    ...
    genarateQuery(column, e, fromData, needQuery) {
        ...
        if (gObj.allowPaging || gObj.enableVirtualization || fromData) {
            ...
            let filteredValue = DataUtil.distinct(e, field, false);
