...
function getFilterMenuPostion(target, dialogObj) {
    ...
    let newpos = calculateRelativeBasedPosition(target, dialogObj.element);
...
/** @hidden */
const columnMenuClick = 'contextMenuClick';
/** @hidden */
...
class CheckBoxFilter {
    ...
    getAllData() {
        ...
        let promise = this.options.dataSource.executeQuery(query);
        promise.then((e) => this.dataSuccess(e));
    }
    dataSuccess(e) {
        ...
        let res = CheckBoxFilter.getDistinct(result, this.options.field);
    ...
    createFilterItems(data, isInitial) {
        ...
        if (data.length) {
            ...
            let isColFiltered = new DataManager(this.options.filteredColumns).executeLocal(new Query().where('field', 'equal', this.options.field)).length;
    ...
    static setDateObject(filterObject) {
        ...
        let prevDate = new Date(value.setDate(value.getDate() - 1));
        let nextDate = new Date(value.setDate(value.getDate() + 2));
    ...
    static getDatePredicate(predicate) {
...
class Data {
    ...
    generateQuery(skipPage) {
        ...
        if (gObj.allowFiltering && gObj.filterSettings.columns.length) {
            ...
            if (gObj.filterSettings.type === 'checkbox' || gObj.filterSettings.type === 'excel') {
                let excelPredicate = CheckBoxFilter.getPredicate(gObj.filterSettings.columns);
                for (let prop of Object.keys(excelPredicate)) {
                    let and = 'and';
                    let obj = excelPredicate[prop];
                    predicate = !isNullOrUndefined(predicate) ?
                        predicate[and](obj) :
                        obj;
                }
                ...
                for (let col of columns) {
                    let sType = gObj.getColumnByField(col.field).type;
        ...
        gObj.aggregates.forEach((row) => {
            row.columns.forEach((column) => {
                let types = column.type instanceof Array ? column.type : [column.type];
                types.forEach((type) => query.aggregate(type, column.field));
            });
        });
        ...
        if (gObj.allowGrouping && gObj.groupSettings.columns.length) {
            ...
            for (let i = 0, len = columns.length; i < len; i++) {
                let isGrpFmt = gObj.getColumnByField(columns[i]).enableGroupByFormat;
                let format = gObj.getColumnByField(columns[i]).format;
                if (isGrpFmt) {
    ...
    formatGroupColumn(value, field) {
        ...
        let column = gObj.getColumnByField(field);
        let date = value;
        if (!column.type) {
...
class RowRenderer {
    ...
    refreshRow(row, columns, attributes$$1, rowTemplate, cloneNode) {
        ...
        for (let i = 0, len = row.cells.length; i < len; i++) {
            ...
            let cellRenderer = cellRendererFact.getCellRenderer(row.cells[i].cellType || CellType.Data);
            let td = cellRenderer.render(row.cells[i], row.data, { 'index': !isNullOrUndefined(row.index) ? row.index.toString() : '' });
...
class RowModelGenerator {
    ...
    generateCells(options) {
        ...
        let tmp = [];
        dummies.forEach((dummy, index) => tmp.push(this.generateCell(dummy, options.uid, isNullOrUndefined(dummy.commands) ? undefined : CellType.CommandColumn, null, index)));
        return tmp;
    }
    generateCell(column, rowId, cellType, colSpan, oIndex) {
        let opt = {
    ...
    refreshRows(input) {
        input.forEach((row) => row.cells = this.generateCells(row));
        return input;
    }
}
...
class SummaryModelGenerator {
    ...
    generateRows(input, args) {
        ...
        this.getData().forEach((row, index) => {
            rows.push(this.getGeneratedRow(row, data[index], args ? args.level : undefined));
        });
        return rows;
    }
    getGeneratedRow(summaryRow, data, raw) {
        let tmp = [];
        ...
        this.getColumns().forEach((value, index) => tmp.push(this.getGeneratedCell(value, summaryRow, index >= indentLength ? this.getCellType() : CellType.Indent, indents[index])));
    ...
    setTemplate(column, data, single) {
        ...
        types.forEach((type) => {
            ...
            let val = group.aggregates && !isNullOrUndefined(group.aggregates[key]) ? group.aggregates[key] :
                calculateAggregate(type, group.aggregates ? group.result : data, column, this.parent);
...
class ContentRender {
    ...
    constructor(parent, serviceLocator) {
        ...
        this.rafCallback = (args) => () => {
            ...
            this.parent.notify(contentReady, { rows: this.rows, args: args });
            if (this.isLoaded) {
    ...
    setSelection(uid, set, clearAll) {
        this.getRows().filter((row) => clearAll || uid === row.uid)
            .forEach((row) => row.isSelected = set);
    }
    getRowByIndex(index) {
...
class HeaderRender {
    ...
    constructor(parent, serviceLocator) {
        this.helper = (e) => {
            ...
            let col = gObj.getColumnByUid(headercelldiv.getAttribute('e-mappinguid'));
    ...
    getHeaderCells(rows) {
        ...
        let cols = this.parent.enableColumnVirtualization ? this.parent.getColumns()
    ...
    appendCells(cols, rows, index, isFirstObj, isFirstCol, isLastCol) {
...
class CellRenderer {
    ...
    refreshCell(cell, data, attributes$$1) {
        ...
        let value = this.getValue(column.field, data, column);
    ...
    buildAttributeFromCell(node, cell) {
        let attr = {};
...
class GroupCaptionCellRenderer extends CellRenderer {
    ...
    render(cell, data) {
        ...
        let gObj = this.parent;
        let result;
        let value = cell.column.enableGroupByFormat ? data.key :
...
class Render {
    ...
    refreshDataManager(args = {}) {
        ...
        let dataManager = this.data.getData(args, this.data.generateQuery().requiresCount());
...
class ColumnWidthService {
    ...
    setWidthToTable() {
        ...
        if (this.parent.frozenColumns) {
            let freezeWidth = 0;
            let colGrp = this.parent.getContentTable().querySelector('colgroup');
            let mColGrp = select('.e-movablecontent');
            for (let i = 0; i < this.parent.getHeaderTable().querySelector('.e-columnheader').children.length; i++) {
            ...
            freezeWidth = 0;
            for (let i = 0; i < select('.e-movableheader').querySelector('.e-columnheader').children.length; i++) {
...
class FocusStrategy {
    ...
    onKeyPress(e) {
        ...
        if (this.swap) {
            let isHeader = this.header === this.getContent();
            this.setActive(isHeader);
            let rowInitVal = isHeader ? -1 : this.header.matrix.matrix.length;
            let current = this.getContent().matrix.get(rowInitVal, bValue[1], [0, 0], e.action, this.getContent().validator());
    ...
    addEventListener() {
        ...
        ['start-edit', 'start-add'].forEach((evt) => this.parent.on(evt, this.clearOutline, this));
...
class ContentFocus {
    ...
    shouldFocusChange(e) {
        ...
        let cell = this.parent.getContentTable().rows[rIndex].cells[cIndex];
...
class HeaderFocus extends ContentFocus {
    ...
    shouldFocusChange(e) {
        ...
        let cell = this.parent.getHeaderTable().rows[rIndex].cells[cIndex];
...
class Selection {
    ...
    selectRows(rowIndexes) {
        let gObj = this.parent;
        let selectedRow = gObj.getRowByIndex(rowIndexes[0]);
    ...
    addRowsToSelection(rowIndexes) {
        ...
        for (let rowIndex of rowIndexes) {
            let data = gObj.getCurrentViewRecords()[rowIndex];
    ...
    updatePersistCollection(selectedRow, chkState) {
        if (this.persistSelection && !isNullOrUndefined(selectedRow)) {
            let rowObj = this.parent.getRowObjectFromUID(selectedRow.getAttribute('data-uid'));
    ...
    clearRowSelection() {
        if (this.isRowSelected) {
            ...
            for (let i = 0, len = this.selectedRowIndexes.length; i < len; i++) {
                let row = this.parent.getRowByIndex(this.selectedRowIndexes[i]);
                if (row) {
    ...
    rowDeselect(type, rowIndex, data, row) {
    ...
    selectCell(cellIndex, isToggle) {
        ...
        let gObj = this.parent;
        let selectedCell = gObj.getCellFromIndex(cellIndex.rowIndex, this.getColIndex(cellIndex.rowIndex, cellIndex.cellIndex));
    ...
    selectCellsByRange(startIndex, endIndex) {
        ...
        let gObj = this.parent;
        let selectedCell = gObj.getCellFromIndex(startIndex.rowIndex, startIndex.cellIndex);
        let min;
        let max;
    ...
    selectCells(rowCellIndexes) {
        ...
        let gObj = this.parent;
        let selectedCell = gObj.getCellFromIndex(rowCellIndexes[0].rowIndex, rowCellIndexes[0].cellIndexes[0]);
    ...
    addCellsToSelection(cellIndexes) {
        ...
        let gObj = this.parent;
        let selectedCell = gObj.getCellFromIndex(cellIndexes[0].rowIndex, this.getColIndex(cellIndexes[0].rowIndex, cellIndexes[0].cellIndex));
        let index;
    ...
    getColIndex(rowIndex, index) {
        let cells = this.parent.getDataRows()[rowIndex].querySelectorAll('td.e-rowcell');
    ...
    cellDeselect(type, cellIndexes, data, cells) {
    ...
    clearCellSelection() {
        if (this.isCellSelected) {
            ...
            for (let i = 0, len = rowCell.length; i < len; i++) {
                ...
                for (let j = 0, cLen = rowCell.length; j < cLen; j++) {
    ...
    checkBoxSelectionChanged() {
        let isCheckColumn = false;
        for (let col of this.parent.columns) {
    ...
    refreshPersistSelection() {
        ...
        if (rows.length > 0 && (this.persistSelection || this.chkField)) {
            ...
            for (let j = 0; j < rows.length; j++) {
                let rowObj = this.parent.getRowObjectFromUID(rows[j].getAttribute('data-uid'));
                let pKey = rowObj ? rowObj.data[this.primaryKey] : null;
                if (pKey === null) {
                    return;
                }
        ...
        if (e.requestType === 'save' && this.persistSelection) {
            ...
            if (!isNullOrUndefined(editChkBox)) {
                ...
                if (row) {
                    ...
                    let rowObj = this.parent.getRowObjectFromUID(row.getAttribute('data-uid'));
                    if (!rowObj) {
                        return;
                    }
    ...
    onDataBound() {
        if (this.persistSelection || this.chkField) {
            if (this.parent.enableVirtualization) {
                let records = this.parent.getCurrentViewRecords();
    ...
    checkSelectAll(checkBox) {
        let state = checkBox.nextElementSibling.classList.contains('e-check');
    ...
    setCheckAllState() {
        if (this.isChkSelection) {
            ...
            let spanEle = this.chkAllBox.nextElementSibling;
    ...
    onCellFocused(e) {
        let clear = ((e.container.isHeader && e.isJump) || (e.container.isContent && !e.container.isSelectable)) &&
            !(e.byKey && e.keyArgs.action === 'space');
        let headerAction = e.container.isHeader && !(e.byKey && e.keyArgs.action === 'space');
        if (!e.byKey || clear) {
            if (clear) {
                this.clearSelection();
            }
...
class ShowHide {
    ...
    batchChanges(handler, columnName, showHide) {
...
class Print {
    ...
    contentReady() {
        ...
        if (gObj.height !== 'auto') {
            ...
            let contentDiv = this.element.querySelector('.e-content');
            let headerDiv = this.element.querySelector('.e-gridheader');
    ...
    removeColGroup(depth) {
        let groupCaption = this.element.querySelectorAll('.e-groupcaption');
        let colSpan = groupCaption[depth - 1].getAttribute('colspan');
        for (let i = 0; i < groupCaption.length; i++) {
            groupCaption[i].setAttribute('colspan', colSpan);
        }
        let colGroups = this.element.querySelectorAll('colgroup');
...
let Grid = class Grid extends Component {
    ...
    getRowInfo(target) {
        ...
        if (!isNullOrUndefined(target) && isNullOrUndefined(parentsUntil(ele, 'e-detailrowcollapse')
            ...
            if (!isNullOrUndefined(cell)) {
                ...
                let rowsObject = this.contentModule.getRows().filter((r) => r.uid === row.getAttribute('data-uid'));
                let rowData = rowsObject[0].data;
                let column = rowsObject[0].cells[cellIndex].column;
    ...
    getRowObjectFromUID(uid) {
        for (let row of this.contentModule.getRows()) {
            if (row.uid === uid) {
                return row;
            }
        }
     ...
     * @return {void}
     */
    filterByColumn(fieldName, filterOperator, filterValue, predicate, matchCase, actualFilterValue, actualOperator) {
            ...
            for (let key of keys) {
                if ((typeof this[key] === 'object') && !isNullOrUndefined(this[key])) {
                    if (Array.isArray(this[key])) {
                        this[key].forEach((element, index) => {
    ...
    excelExport(exportProperties, isMultipleExport, workbook) {
     ...
     */
    /* tslint:disable-next-line:no-any */
    csvExport(exportProperties, isMultipleExport, workbook) {
     ...
     */
    /* tslint:disable-next-line:no-any */
    pdfExport(exportProperties, isMultipleExport, pdfDoc) {
...
class Page {
    ...
    canSkipAction(action) {
        let page = {
            pageUp: (el) => el.scrollTop !== 0,
            pageDown: (el) => !(el.scrollTop >= el.scrollHeight - el.clientHeight)
        };
...
class ExcelFilter extends CheckBoxFilter {
    ...
    createMenu(type, isFiltered, isCheckIcon) {
        ...
        let icon = isFiltered ? 'e-icon-filter e-filtered' : 'e-icon-filter';
    ...
    hoverHandler(e) {
        ...
        let selectedMenu;
        let predicates = this.getExistingPredicate(this.options.field);
    ...
    openDialog(options) {
        ...
        let filterLength = this.getExistingPredicate(options.field).length;
    ...
    renderCustomFilter(target, column) {
        ...
        //column name
        let fieldSet = createElement('div', { innerHTML: column, className: 'e-xlfl-fieldset' });
     ...
     */
    filterByColumn(fieldName, firstOperator, firstValue, predicate, ignoreCase, secondOperator, secondValue) {
        let fColl = [];
        let mPredicate;
        if (firstValue) {
            fColl.push({
    ...
    getExistingPredicate(column) {
        let cols = CheckBoxFilter.getDistinct(this.options.filteredColumns, 'field').records;
    ...
    renderFilterUI(column, dlgConetntEle) {
        let predicates = this.getExistingPredicate(column);
        let table = createElement('table', { className: 'e-xlfl-tabel' });
    ...
    renderRadioButton(column, tr, predicates) {
        let td = createElement('td', { className: 'e-xlfl-radio' });
        tr.appendChild(td);
        let radioDiv = createElement('div', { className: 'e-xlfl-radiodiv' });
        /* tslint:disable-next-line:max-line-length */
        ...
        let flValue = predicates.length === 2 ? predicates[1].predicate : 'and';
        if (flValue === 'and') {
    ...
    renderMatchCase(column, tr, elementId, predicates) {
        let matchCase = createElement('td', { className: 'e-xlfl-mtcase' });
        tr.appendChild(matchCase);
        let matchCaseDiv = createElement('div', { className: 'e-xlfl-matchcasediv' });
        let matchCaseInput = createElement('input', { id: column + elementId, attrs: { 'type': 'checkbox' } });
        matchCaseDiv.appendChild(matchCaseInput);
        matchCase.appendChild(matchCaseDiv);
        let flValue = predicates.length > 0 ?
...
class Filter {
    ...
    updateModel() {
        ...
        let index = this.getFilteredColsIndexByField(this.fieldName);
        if (index > -1) {
    ...
    getFilteredColsIndexByField(field) {
    ...
    removeFilteredColsByField(field, isClearFilterBar) {
        ...
        for (let i = 0, len = cols.length; i < len; i++) {
            if (cols[i].field === field) {
                ...
                cols.splice(i, 1);
                let fltrElement = this.parent.getColumnHeaderByField(field);
    ...
    onTimerTick() {
        let filterElement = this.element.querySelector('#' + this.column.field + '_filterBarcell');
    ...
    columnPositionChanged(e) {
        ...
        let filterCells = [].slice.call(this.element.querySelectorAll('.e-filterbarcell'));
...
class Resize {
    ...
    resizeColumn(fName, index, id) {
        let gObj = this.parent;
        let tWidth = 0;
        let headerTable = this.parent.getHeaderTable();
        let contentTable = this.parent.getContentTable();
        let headerDivTag = 'e-gridheader';
        let contentDivTag = 'e-gridcontent';
        let indentWidthClone = gObj.getHeaderTable().querySelector('tr').querySelectorAll('.e-grouptopleftcell');
        let indentWidth = 0;
        ...
        let headerTextClone = headerTable.querySelectorAll('th')[columnIndex].cloneNode(true);
        let headerText = [headerTextClone];
        let contentTextClone = contentTable.querySelectorAll(`td:nth-child(${columnIndex + 1})`);
        let contentText = [];
        ...
        if (!this.parent.allowResizing) {
            ...
            let tableWidth = tWidth + indentWidth;
            if (tWidth > 0) {
    ...
    getScrollBarWidth(height) {
        let ele = this.parent.frozenColumns ? this.parent.getContent().querySelector('.e-movablecontent')
...
class Reorder {
    ...
    chkDropAllCols(srcElem, destElem) {
        let isFound;
        let headers = [].slice.call(this.parent.element.getElementsByClassName('e-headercell'));
        let header;
    ...
    keyPressHandler(e) {
        ...
        switch (e.action) {
            ...
            case 'enter':
                ...
                let row = element.parentElement.querySelector('[class^="e-record"]');
                if (!row) {
                    break;
                }
...
class Toolbar$1 {
    ...
    render() {
        ...
        for (let item of preItems) {
            let localeName = item[0].toUpperCase() + item.slice(1);
...
class FooterRenderer extends ContentRender {
    ...
    renderTable() {
        let contentDiv = this.getPanel();
        let innerDiv = this.createContentTable();
    ...
    renderSummaryContent(e) {
        ...
        let dummies = this.modelGenerator.getColumns();
        let rows = this.modelGenerator.generateRows(input, e || this.aggregates);
    ...
    refreshCol() {
        let headerCol = this.parent.element.querySelector('.e-gridheader').querySelector('colgroup').cloneNode(true);
...
class VirtualRowModelGenerator {
    ...
    checkAndResetCache(action) {
        let clear = ['paging', 'refresh', 'sorting', 'filtering', 'searching', 'grouping', 'ungrouping']
...
class DialogEditRender {
    ...
    getEditElement(elements) {
        ...
        for (let i = 0; i < cols.length; i++) {
            ...
            let tr = createElement('tr');
            let dataCell = createElement('td', { className: 'e-rowcell', attrs: { style: 'text-align:left;width:190px' } });
...
class DropDownEditCell {
    ...
    ddActionComplete(e) {
        e.result = e.result.filter((val, i, values) => values.indexOf(val) === i);
        e.result.sort();
    }
    dropDownOpen(args) {
...
class BatchEdit {
    ...
    onCellFocused(e) {
        let clear = !e.container.isContent || !e.container.isDataCell;
        if (!e.byKey || clear) {
            return;
        }
    ...
    refreshRowIdx() {
        let rows = [].slice.call(this.parent.getContentTable().querySelector('tbody').children);
    ...
    editCell(index, field, isAdd) {
        ...
        if (gObj.editSettings.allowEditing && col.allowEditing) {
            ...
            let row = gObj.getDataRows()[index];
...
class Edit {
    ...
    destroyToolTip() {
        let elements = [].slice.call(this.parent.getContentTable().querySelectorAll('.e-griderror'));
        for (let elem of elements) {
            remove(elem);
        }
    }
    ...
    valErrorPlacement(inputElement, error) {
        if (this.parent.isEdit) {
            ...
            let parentElem = this.parent.editSettings.mode !== 'dialog' ? this.parent.getContentTable() :
                this.parent.element.querySelector('#' + this.parent.element.id + '_dialogEdit_wrapper');
            let elem = parentElem.querySelector('#' + id + '_Error');
            if (!elem) {
    ...
    validationComplete(args) {
        if (this.parent.isEdit) {
            let parentElem = this.parent.editSettings.mode !== 'dialog' ? this.parent.getContentTable() :
                this.parent.element.querySelector('#' + this.parent.element.id + '_dialogEdit_wrapper');
            let elem = parentElem.querySelector('#' + args.inputName + '_Error');
            if (elem) {
    ...
    createTooltip(element, error, name, display) {
        let table = this.parent.editSettings.mode !== 'dialog' ? this.parent.getContentTable() :
        ...
        let inputClient = parentsUntil(element, 'e-rowcell').getBoundingClientRect();
        let div = createElement('div', {
...
class ExcelExport {
    ...
    /* tslint:disable-next-line:max-line-length */
    /* tslint:disable-next-line:no-any */
    Map(grid, exportProperties, isMultipleExport, workbook, isCsv) {
        let gObj = grid;
    ...
    processRecords(gObj, exportProperties, isMultipleExport, workbook) {
        if (exportProperties !== undefined && exportProperties.dataSource !== undefined &&
            ...
            let promise;
            return promise = new Promise((resolve, reject) => {
                let dataManager = this.data.getData({}, ExportHelper.getQuery(gObj, this.data));
                dataManager.then((r) => {
                    this.init(gObj);
                    this.processInnerRecords(gObj, exportProperties, isMultipleExport, workbook, r);
                    resolve(this.book);
    ...
    processRecordRows(gObj, record, headerRow, level) {
        ...
        for (let r = 0; r < rLen; r++) {
            ...
            for (let c = 0, len = headerRow.columns.length; c < len; c++) {
                /* tslint:disable-next-line:no-any */
                let value = record[r][headerRow.columns[c].field];
                if (!isNullOrUndefined(value)) {
                    /* tslint:disable-next-line:no-any */
                    let excelCellArgs = { data: record[r], column: headerRow.columns[c] };
    ...
    /* tslint:disable-next-line:no-any */
    processAggregates(gObj, rec, currentViewRecords, indent) {
...
class PdfExport {
     ...
     */
    /* tslint:disable-next-line:no-any */
    Map(parent, pdfExportProperties, isMultipleExport, pdfDoc) {
        ...
        if (pdfExportProperties !== undefined && pdfExportProperties.dataSource !== undefined && pdfExportProperties.dataSource instanceof DataManager) {
            ...
            let promise;
            return promise = new Promise((resolve, reject) => {
                let dataManager = this.data.getData({}, ExportHelper.getQuery(parent, this.data));
                dataManager.then((returnType) => {
                    this.init(parent);
                    if (pdfDoc !== undefined) {
                        this.pdfDocument = pdfDoc;
                    }
    ...
    getSummaryCaptionThemeStyle() {
        if (this.gridTheme !== undefined && this.gridTheme.caption !== undefined && this.gridTheme.caption !== null) {
            let fontSize = this.gridTheme.caption.fontSize !== undefined ? this.gridTheme.caption.fontSize : 9.75;
    ...
    getHeaderThemeStyle() {
        ...
        if (this.gridTheme !== undefined && this.gridTheme.header !== undefined && this.gridTheme.header !== null) {
            ...
            let fontSize = this.gridTheme.header.fontSize !== undefined ? this.gridTheme.header.fontSize : 10.5;
            let pdfColor = new PdfColor();
    ...
    drawImage(documentHeader, content) {
        let x = content.position.x * 0.75;
        let y = content.position.y * 0.75;
        let width = (content.size !== undefined) ? (content.size.width * 0.75) : undefined;
        let height = (content.size !== undefined) ? (content.size.height * 0.75) : undefined;
    ...
    processRecord(border, columns, gObj, dataSource, pdfGrid, groupIndex) {
        ...
        for (let items of dataSource) {
            ...
            for (let j = 0; j < columns.length; j++) {
                /* tslint:disable:no-any */
                let value = items[columns[j].field];
                let data = items;
                let args = {
                    data: data,
                    value: value,
    ...
    processCellStyle(cell, args) {
        ...
        if (args.style.border !== undefined) {
            ...
            // set border width
            let width = (borderWidth !== undefined && typeof borderWidth === 'number') ? (borderWidth * 0.75) : (undefined);
            // set border color
    ...
    /* tslint:disable-next-line:no-any */
    getFont(content) {
        let fontSize = (content.style.fontSize !== undefined) ? (content.style.fontSize * 0.75) : 9.75;
        /* tslint:disable-next-line:max-line-length */
        let fontFamily = (content.style.fontFamily !== undefined) ? (this.getFontFamily(content.style.fontFamily)) : PdfFontFamily.Helvetica;
...
class CommandColumn {
    ...
    commandClickHandler(e) {
        let gObj = this.parent;
        let gID = this.parent.element.id;
...
class FreezeRender extends HeaderRender {
    ...
    setWrapHeight(fRows, mRows, isModeChg, isContReset) {
        let fRowHgt;
        let mRowHgt;
